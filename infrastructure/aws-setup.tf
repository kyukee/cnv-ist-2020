...
data "aws_ami" "autoscaler-ami" {
    ...
    filter {
        name   = "tag:Name"
        values = ["Packer-CNV-AutoScaler-*"] // defined in the corresponding packer ami definition file
    }
...
resource "aws_instance" "autoscaler-instance" {
    count                  = 1
    ami                    = data.aws_ami.autoscaler-ami.id
    instance_type          = "t2.micro"
    availability_zone      = "us-east-1a"
    monitoring             = true
    vpc_security_group_ids = ["${aws_security_group.security-group.id}"]
    ...
    tags = {
        Name = format("autoscaler-%03d", count.index + 1)
    }
}
